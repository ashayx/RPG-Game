...
class ObjectPool{
     ...
     *
     */
    public static pop(cls: any, ...args: any[]): any{
        App.GlobalObject.addGlobalObj(cls);
        const refKey = egret.getQualifiedClassName(cls);
    ...
    public static push(obj:any):boolean{
        ...
        if(ObjectPool._content[refKey].indexOf(obj) != -1) {
            console.warn("对象池重复添加，检查应用层逻辑。");
            return false;
        }
