...
class WinsManager {
	...
	public switchWin(cls: any):void{
		...
		if(win.stage==null)
			this.openWindow(cls);
			// DisplayUtility.addChild(App.LayerManager.EFFECT_LAYER, cls)
		else
			this.closeWin(cls);
			// DisplayUtility.removeFromParent( cls)
	}
	...
	public openWindow(cls: any): void {
		...
		if (!win.stage) {
			...
			// }
			DisplayUtility.addChild(App.LayerManager.EFFECT_LAYER, win)
		}
	}
	...
	public closeWin(target: any): void {
		...
		var win = this._windowMap.get(target);
		DisplayUtility.removeFromParent( win )
	}
